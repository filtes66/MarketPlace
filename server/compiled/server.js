...
client.connect(err => {
  ...
  routes.get("/photos", function (req, res) {
    console.log("photos");
    photos.find().toArray().then((error, results) => {
      if (error) {
  ...
  routes.post("/photos/add", jsonParser, function (req, res) {
    photos.insertOne(req.body).then(() => res.status(200).send("successfully inserted new document")).catch(err => {
      console.log(err);
      res.send(err);
    });
  });
  ...
  routes.post("/users/add", jsonParser, function (req, res) {
    users.insertOne(req.body).then(() => res.status(200).send("successfully inserted new document")).catch(err => {
      console.log(err);
      res.send(err);
    });
  });
  routes.post("/orders/add", jsonParser, function (req, res) {
    orders.insertOne(req.body).then(() => res.status(200).send("successfully inserted new document")).catch(err => {
      console.log(err);
      res.send(err);
    });
  });
